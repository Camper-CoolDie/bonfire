...
object ControllerAccounts {
    ...
    fun remove(
        ...
        // user events (may have been removed by removePublications)
        if (!removePublications) {
            Database.remove("ControllerAccounts.remove.removeEvents", SqlQueryRemove(TPublications.NAME)
        ...
        // account
        Database.update("ControllerAccounts.remove", SqlQueryUpdate(TAccounts.NAME)
            .where(TAccounts.id, "=", accountId)
            .updateValue(TAccounts.name, "deleted#${accountId}")
            .updateValue(TAccounts.google_id, "")
            .updateValue(TAccounts.img_id, 0L)
            .updateValue(TAccounts.img_title_id, 0L)
            .updateValue(TAccounts.img_title_gif_id, 0L)
            .updateValue(TAccounts.sex, 0L)
            .updateValue(TAccounts.ban_date, 0L)
            .updateValue(TAccounts.recruiter_id, 0L)
            .updateValue(TAccounts.lvl, 100L)
            .updateValue(TAccounts.karma_count_30, 0L)
            .updateValue(TAccounts.karma_count_total, 0L)
            .updateValue(TAccounts.last_online_time, 0L)
            .updateValue(TAccounts.subscribes, "")
            .updateValue(TAccounts.refresh_token, "")
            .updateValue(TAccounts.refresh_token_date_create, 0L)
            .updateValue(TAccounts.reports_count, 0L)
            .updateValue(TAccounts.account_settings, ""))
    }
